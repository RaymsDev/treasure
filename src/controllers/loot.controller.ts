...
export class LootController {
  ...
  public static Rolled(lootTable : IRarityTable, rariry:string ,dieRolled: number){
    let result = null;
    const loots = lootTable[rariry];
    
    for (let index = 0; index < loots.length; index++) {
      const loot = loots[index];
      if(beetweenHelper.IsBetween(dieRolled, loot.diceResult.less, loot.diceResult.high)){
        result = loot;
        break;
      }
    }
