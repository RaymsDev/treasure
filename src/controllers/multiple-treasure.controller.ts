...
import {
  IMultipleTreasure,
  ILoot,
  IMultipleTreasureResult
} from "./../models/multiple-treasure.model";
...
export class MultipleTrasureController {
  ...
  private static rollTreasures(
    ...
    if (multipleTreasure) {
      ...
      if (multipleTreasure.gems) {
        result.gems = MultipleTrasureController.rollGems(
          GEMS[multipleTreasure.gems.rarity],
          multipleTreasure.gems.rarity,
          multipleTreasure.gems.roll
        );
      }

      if (multipleTreasure.pieceOfArts) {
        result.pieceOfArts = MultipleTrasureController.rollPieceOfArts(
          PIECE_OF_ARTS[multipleTreasure.pieceOfArts.rarity],
          multipleTreasure.pieceOfArts.rarity,
          multipleTreasure.pieceOfArts.roll
        );
      }

      if (multipleTreasure.magicObjects) {
        result.magicObjects = MultipleTrasureController.rollMagicObjects(
          MAGIC_OBJECTS[multipleTreasure.magicObjects.rarity],
          multipleTreasure.magicObjects.rarity,
          multipleTreasure.magicObjects.roll
        );
      }
    }
  ...
  private static rollPieceOfArts(
    ...
    if (roll) {
      ...
      const rolls = diceRoller.Roll(
        pieceOfArts[0].diceResult.dieType,
        rollNumber
      );
  ...
  private static rollMagicObjects(
    magicOfObjects: Array<IItem>,
    rarity: string,
    roll?: IRoll
  ): Array<IItem> {
    let results = new Array<IItem>();
    ...
    if (roll) {
      ...
      const rolls = diceRoller.Roll(
        magicOfObjects[0].diceResult.dieType,
        rollNumber
      );
      ...
      magicOfObjects.forEach(mo => {
        rolls.forEach(r => {
          if (
            beetweenHelper.IsBetween(r, mo.diceResult.less, mo.diceResult.high)
          ) {
            results.push(mo);
          }
        });
